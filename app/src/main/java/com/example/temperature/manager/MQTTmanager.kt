package com.example.temperature.manager
...
class MQTTmanager (val connectionParams: MQTTConnectionParams, val context: Context, val uiUpdater: UIUpdaterInterface?) {
    ...
    fun publish(tpic:String,message:String){
        try
        {
            var msg = message
            client.publish(tpic,msg.toByteArray(),0,false,null,object :IMqttActionListener{
                override fun onSuccess(asyncActionToken: IMqttToken?) {
